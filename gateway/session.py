...
class SessionSource:
    ...
    gateway_bot_id: Optional[str] = None  # Platform adapter instance id (Telegram multi-bot)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.gateway_bot_id:
            d["gateway_bot_id"] = self.gateway_bot_id
        return d
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionSource":
        return cls(
            ...
            gateway_bot_id=data.get("gateway_bot_id"),
        )
...
def build_session_key(
    ...
    bot_suffix: list[str] = []
    if source.platform == Platform.TELEGRAM and source.gateway_bot_id:
        safe_bot_id = "".join(
            ch if (ch.isalnum() or ch in "._-") else "_"
            for ch in str(source.gateway_bot_id).strip()
        )
        if safe_bot_id:
            bot_suffix = ["bot", safe_bot_id]
    ...
    if source.chat_type == "dm":
        ...
        if dm_chat_id:
            key_parts = ["agent:main", platform, "dm", dm_chat_id]
            if source.thread_id:
                key_parts.append(str(source.thread_id))
            key_parts.extend(bot_suffix)
            return ":".join(key_parts)
        if source.thread_id:
            key_parts = ["agent:main", platform, "dm", str(source.thread_id)]
            key_parts.extend(bot_suffix)
            return ":".join(key_parts)
