...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _gateway_bot_id(self) -> Optional[str]:
        raw = (self.config.extra or {}).get("gateway_bot_id")
        value = str(raw).strip() if raw is not None else ""
        return value or None

    @staticmethod
    def _metadata_gateway_bot_id(metadata: Optional[Dict[str, Any]]) -> Optional[str]:
        if not metadata:
            return None
        raw = metadata.get("telegram_gateway_bot_id") or metadata.get("gateway_bot_id")
        value = str(raw).strip() if raw is not None else ""
        return value or None

    def _child_adapter_for_metadata(
        self,
        metadata: Optional[Dict[str, Any]],
    ) -> Optional["TelegramAdapter"]:
        """Return the multi-bot child adapter that should own this send."""
        bot_id = self._metadata_gateway_bot_id(metadata)
        if not bot_id:
            return None
        for adapter in getattr(self, "_bot_adapters", {}).values():
            if adapter is self:
                continue
            if adapter._gateway_bot_id() == bot_id:
                return adapter
        return None
    ...
    def _model_picker_state_key(
        ...
        bot_id = self._metadata_gateway_bot_id(metadata) or self._gateway_bot_id()
        if bot_id:
    ...
    async def _apply_default_soul(self, event: MessageEvent) -> None:
        ...
        if not chat_id:
            return
        gateway_bot_id = getattr(getattr(event, "source", None), "gateway_bot_id", None)
        soul_chat_id = f"{gateway_bot_id}:{chat_id}" if gateway_bot_id else str(chat_id)
        try:
            from hermes_cli.soul_router import SoulRouter
            router = SoulRouter()
            active = router.get_active_soul(soul_chat_id)
            if active.name == "default":
                router.set_active_soul(soul_chat_id, default_soul)
        except Exception as exc:
    ...
    async def send(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send(chat_id, content, reply_to=reply_to, metadata=metadata)
    ...
    async def send_or_update_status(
        ...
        *,
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_or_update_status(
                chat_id,
                status_key,
                content,
                metadata=metadata,
    ...
    async def edit_message(
        ...
        *,
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.edit_message(
                chat_id,
                message_id,
                content,
                finalize=finalize,
    ...
    async def send_draft(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_draft(
                chat_id,
                draft_id,
                content,
                metadata=metadata,
    ...
    async def send_update_prompt(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_update_prompt(
                chat_id,
                prompt,
                default=default,
    ...
    async def send_exec_approval(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_exec_approval(
                chat_id,
                command,
                session_key,
    ...
    async def send_slash_confirm(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_slash_confirm(
                chat_id,
                title,
                message,
                session_key,
    ...
    async def send_clarify(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_clarify(
                chat_id,
                question,
                choices,
                clarify_id,
    ...
    async def send_model_picker(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_model_picker(
                chat_id,
                providers,
                current_model,
                current_provider,
        ...
        try:
            ...
            # Store picker state per bot+chat. Telegram chat_id is identical
            # across multiple bots in the same DM, so chat_id alone collides.
            state_key = self._model_picker_state_key(str(chat_id), metadata)
    ...
    async def _handle_model_picker_callback(
        ...
        state_key = self._model_picker_state_key(str(chat_id))
            ...
            if len(parts) == 3:
                ...
                if not session_key:
                    try:
                        ...
                        session_key = _slash_confirm_mod.find_session(confirm_id)
                    except Exception as exc:
                        logger.debug(
                            "[%s] slash-confirm registry lookup failed for %s: %s",
                            self.name,
                            confirm_id,
                            exc,
                        )
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_voice(
                chat_id,
                audio_path,
                caption=caption,
                reply_to=reply_to,
    ...
    async def send_multiple_images(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            await delegate.send_multiple_images(
                chat_id,
                images,
                metadata=metadata,
    ...
    async def send_image_file(
        ...
        **kwargs,
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_image_file(
                chat_id,
                image_path,
                caption=caption,
                reply_to=reply_to,
    ...
    async def send_document(
        ...
        **kwargs,
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_document(
                chat_id,
                file_path,
                caption=caption,
                file_name=file_name,
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_video(
                chat_id,
                video_path,
                caption=caption,
                reply_to=reply_to,
    ...
    async def send_image(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_image(
                chat_id,
                image_url,
                caption=caption,
                reply_to=reply_to,
    ...
    async def send_animation(
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            return await delegate.send_animation(
                chat_id,
                animation_url,
                caption=caption,
                reply_to=reply_to,
    ...
    async def send_typing(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        delegate = self._child_adapter_for_metadata(metadata)
        if delegate is not None:
            await delegate.send_typing(chat_id, metadata=metadata)
            return
    ...
    def _build_message_event(
        ...
        source = self.build_source(
            ...
            gateway_bot_id=self._gateway_bot_id(),
        )
