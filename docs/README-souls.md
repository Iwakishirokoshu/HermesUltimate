...
For `backend: decepticon`, Hermes binds each chat session to a stable
Decepticon engagement slug (`hermes-<session-id>`) and passes
`engagement_name` plus `workspace_path=/workspace/<slug>` through LangGraph
`config.configurable`. The Decepticon sandbox mounts
`$HERMES_VAULT_PATH/Engagements` at `/workspace`, so OPPLAN, findings, and
reports persist under `HermesVault/Engagements/<slug>/`.
...
HermesUltimate keeps persistent memory vault-only.

- `Wiki/Hot` is the always-near tier. Souls preload explicit Hot paths through
  `vault_load.include`.
- `Wiki/Warm` is the working reference tier. Use `vault.search` or `vault.list`
  with `tier: "warm"` when the soul needs it.
- `Wiki/Cold` is the archive tier. Use `tier: "cold"` for recall without
  keeping archive notes in every prompt.
- `Wiki/Findings` is the security findings tier and is available through
  `tier: "findings"` for red-team souls.

Soul command routing lives in `gateway/run.py`; the `souls/*.yaml` files define
identity, backend, allowed toolsets, and vault preload paths.
