...
```bash
curl -fsSL https://raw.githubusercontent.com/Iwakishirokoshu/HermesUltimate/main/install \
  | bash -s -- --defaults
```

The default `slim` profile generates `~/.hermes/stack.env`, starts the core
Docker stack plus Decepticon/LangGraph/Neo4j, disables 9router password login,
and binds service ports to `127.0.0.1` on the VPS.

For a manual server install, run the one-command wizard and pick the pieces
you want:
...
If the repository is private, pass a token to both the first raw download and
...
curl -fsSL -H "Authorization: Bearer $HERMES_GITHUB_TOKEN" \
  ...
  | HERMES_GITHUB_TOKEN="$HERMES_GITHUB_TOKEN" bash
```
