...
def find_session(confirm_id: str) -> Optional[str]:
    """Return the session key that owns ``confirm_id``, if still pending."""
    wanted = str(confirm_id)
    with _lock:
        for session_key, entry in _pending.items():
            if str(entry.get("confirm_id")) == wanted:
                return session_key
    return None
