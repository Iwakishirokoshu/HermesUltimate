...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Iwakishirokoshu/HermesUltimate/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:Iwakishirokoshu/HermesUltimate.git"
REPO_URL_HTTPS="https://github.com/Iwakishirokoshu/HermesUltimate.git"
DEFAULT_REPO_URL_HTTPS="$REPO_URL_HTTPS"
...
INSTALL_MODE="${HERMES_INSTALL_MODE:-local}"
STACK_PROFILE="${HERMES_STACK_PROFILE:-}"
RUN_STACK=false
STACK_VAULT_PATH="${HERMES_VAULT_PATH:-$HOME/HermesVault}"
VNC_PASSWORD_VALUE="${VNC_PASSWORD:-}"
WITH_TAILSCALE=false
WITH_SECOND_BOT=false
RUN_POST_INSTALL_WIZARD=true
    ...
    case $1 in
        ...
        --mode)
            ...
            case "$INSTALL_MODE" in
                vps|local) ;;
                *) echo "Unknown install mode: $INSTALL_MODE"; exit 1 ;;
            esac
            shift 2
            ;;
        --profile)
            ...
            case "$STACK_PROFILE" in
                ...
                *) echo "Unknown stack profile: $STACK_PROFILE"; exit 1 ;;
            esac
            RUN_STACK=true
            shift 2
            ;;
        ...
        --vault-path)
            STACK_VAULT_PATH="$2"
            export HERMES_VAULT_PATH="$STACK_VAULT_PATH"
            shift 2
            ;;
        --vnc-password)
            VNC_PASSWORD_VALUE="$2"
            export VNC_PASSWORD="$VNC_PASSWORD_VALUE"
            shift 2
            ;;
        --with-tailscale)
            WITH_TAILSCALE=true
            ...
            ;;
        --with-second-bot)
            WITH_SECOND_BOT=true
            shift
            ;;
        --defaults|--yes|-y)
            NON_INTERACTIVE=true
            RUN_SETUP=false
            RUN_POST_INSTALL_WIZARD=false
            shift
            ;;
        --commit|-Commit)
            INSTALL_COMMIT="$2"
        ...
        -h|--help)
            ...
            echo "  --repo-url URL  Git repository URL (default: HermesUltimate origin)"
            echo "  --mode MODE    Install mode: local|vps (vps defaults to /opt/hermes-ultimate)"
            echo "  --profile NAME Start Docker stack profile: slim|ultra-slim|full"
            echo "  --skip-stack   Do not generate stack env or start Docker Compose"
            echo "  --vault-path PATH  HermesVault path for Docker services"
            echo "  --vnc-password PW  VNC password for browser cloak"
            echo "  --with-tailscale   Install/enable Tailscale SSH via scripts/install-tailscale.sh"
            echo "  --with-second-bot  Include second Telegram bot prompts/templates"
            echo "  --commit SHA   Pin checkout to a specific commit after clone/update"
            echo "  --manifest     Print desktop bootstrap stage manifest as JSON"
            echo "  --stage NAME   Run one desktop bootstrap stage"
            echo "  --json         Print a JSON result frame for --stage"
            echo "  --non-interactive  Skip stages that require user input"
            echo "  --defaults     Use unattended defaults (alias for non-interactive defaults)"
...
resolve_install_layout() {
    ...
    if [ "$INSTALL_MODE" = "vps" ]; then
        INSTALL_DIR="/opt/hermes-ultimate"
        INSTALL_DIR_EXPLICIT=true
        log_info "VPS install layout"
        log_info "  Code:  $INSTALL_DIR"
        log_info "  Data:  $HERMES_HOME"
        return 0
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/Iwakishirokoshu/HermesUltimate/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_system_packages() {
    ...
    if [ "$DISTRO" = "ubuntu" ] || [ "$DISTRO" = "debian" ]; then
        log_info "Checking native Cloak desktop dependencies..."
        for cmd in Xvfb x11vnc openbox websockify xclip; do
            if ! command -v "$cmd" >/dev/null 2>&1; then
                need_cloak_native=true
                break
            fi
        done
        ...
        if [ "$need_cloak_native" = false ]; then
            log_success "Native Cloak desktop dependencies found"
        fi
    fi
    ...
    if [ "$need_ripgrep" = false ] && [ "$need_ffmpeg" = false ] && [ "$need_cloak_native" = false ]; then
        return 0
    fi
    ...
    if [ "$need_cloak_native" = true ]; then
        desc_parts+=("native Cloak desktop dependencies")
        local libasound_pkg="libasound2"
        if command -v apt-cache >/dev/null 2>&1; then
            local libasound_candidate=""
            local libasound_t64_candidate=""
            libasound_candidate="$(apt-cache policy libasound2 2>/dev/null | awk '/Candidate:/ {print $2; exit}')"
            libasound_t64_candidate="$(apt-cache policy libasound2t64 2>/dev/null | awk '/Candidate:/ {print $2; exit}')"
            if { [ -z "$libasound_candidate" ] || [ "$libasound_candidate" = "(none)" ]; } && [ -n "$libasound_t64_candidate" ] && [ "$libasound_t64_candidate" != "(none)" ]; then
...
clone_repo() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        ...
        if [ "$REPO_URL" != "$DEFAULT_REPO_URL_HTTPS" ]; then
...
install_deps() {
    ...
    install_native_cloakbrowser() {
        log_info "Installing native CloakBrowser package for dashboard profiles..."
        if $UV_CMD pip install "cloakbrowser[geoip]==0.3.31"; then
            log_success "Native CloakBrowser package installed"
            return 0
        fi
        log_error "Could not install cloakbrowser[geoip]; native Cloak profiles would not launch."
        log_info "Manual recovery: $UV_CMD pip install 'cloakbrowser[geoip]==0.3.31'"
        exit 1
    }
...
attempt_install_docker() {
    ...
    log_info "Docker not found; attempting apt install for Docker Engine/Compose..."
    apt-get update

    local pkgs=(docker.io)
...
ensure_docker_stack_prereqs() {
    if [ "$RUN_STACK" != true ]; then
        return 0
    fi
    if [ "$DISTRO" = "termux" ]; then
        log_warn "Skipping Docker stack on Termux"
        RUN_STACK=false
        return 0
    fi
    ...
    if ! command -v docker >/dev/null 2>&1; then
        attempt_install_docker || {
            log_error "Docker is required for the Hermes Ultimate stack."
            log_info "Install Docker Engine with Compose v2, then rerun with --profile $STACK_PROFILE."
            exit 1
        }
    fi
    ...
    if ! docker info >/dev/null 2>&1; then
        ...
        log_info "Start Docker and rerun the installer."
        exit 1
    fi

    if ! docker_compose version >/dev/null 2>&1; then
        log_error "Docker Compose v2 (or docker-compose) is required."
        log_info "Install the Docker Compose plugin and rerun the installer."
        exit 1
    fi
}
...
stack_compose_args() {
    ...
    case "$STACK_PROFILE" in
        ...
        ultra-slim|"")
            ;;
        *)
            log_error "Unknown stack profile: $STACK_PROFILE"
            exit 1
            ;;
    esac
}
...
prepare_stack_env() {
    ...
    STACK_VAULT_PATH="$(expand_user_path "$STACK_VAULT_PATH")"
    mkdir -p "$STACK_VAULT_PATH" "$STACK_VAULT_PATH/Engagements"

    local gen_args=("--vault-path" "$STACK_VAULT_PATH")
    if [ -n "$VNC_PASSWORD_VALUE" ]; then
        gen_args+=("--vnc-password" "$VNC_PASSWORD_VALUE")
    fi

    HERMES_STACK_ENV="${HERMES_STACK_ENV:-$HERMES_HOME/stack.env}" \
    HERMES_VAULT_PATH="$STACK_VAULT_PATH" \
    bash "$INSTALL_DIR/scripts/gen-env.sh" "${gen_args[@]}"
}
...
start_stack() {
    ...
    while IFS= read -r item; do
        compose_args+=("$item")
    done < <(stack_compose_args)

    log_info "Validating Docker Compose stack ($STACK_PROFILE)..."
    docker_compose "${compose_args[@]}" config >/dev/null

    log_info "Starting Docker Compose stack ($STACK_PROFILE)..."
    docker_compose "${compose_args[@]}" up -d --build

    if [ -x "$INSTALL_DIR/scripts/disable-9router-auth.sh" ]; then
        log_info "Disabling 9Router password login..."
        "$INSTALL_DIR/scripts/disable-9router-auth.sh" || {
            log_warn "Could not disable 9Router auth now; retry manually: $INSTALL_DIR/scripts/disable-9router-auth.sh"
        }
    fi
}
...
maybe_install_tailscale() {
    ...
    if [ ! -x "$INSTALL_DIR/scripts/install-tailscale.sh" ]; then
        log_warn "Tailscale installer not found; skipping"
        return 0
    fi
    log_info "Installing/enabling Tailscale SSH..."
    "$INSTALL_DIR/scripts/install-tailscale.sh" || log_warn "Tailscale setup failed; continue without it"
}

run_post_install_wizard() {
    if [ "$RUN_POST_INSTALL_WIZARD" != true ]; then
        log_info "Skipping Hermes Ultimate post-install wizard"
        return 0
    fi
    if [ "$RUN_STACK" != true ]; then
        return 0
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Post-install wizard skipped (no terminal available). Run scripts/post-install-wizard.sh later."
        return 0
    fi

    local wizard_args=()
    if [ "$WITH_SECOND_BOT" = true ]; then
        wizard_args+=("--with-second-bot")
    fi

    log_info "Starting Hermes Ultimate post-install wizard..."
    HERMES_STACK_ENV="${HERMES_STACK_ENV:-$HERMES_HOME/stack.env}" \
    HERMES_VAULT_PATH="$STACK_VAULT_PATH" \
    bash "$INSTALL_DIR/scripts/post-install-wizard.sh" "${wizard_args[@]}" < /dev/tty
}
...
run_setup_wizard() {
    ...
    if [ "$NON_INTERACTIVE" = true ]; then
        log_info "Setup wizard skipped (--non-interactive)"
        return 0
    fi
