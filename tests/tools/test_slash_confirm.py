...
class TestRegisterAndGetPending:
    ...
    def test_find_session_by_confirm_id(self):
        async def h(choice):
            return "x"

        slash_confirm.register("sess1", "cid1", "cmd", h)
        slash_confirm.register("sess2", "cid2", "cmd", h)

        assert slash_confirm.find_session("cid2") == "sess2"
        assert slash_confirm.find_session("missing") is None
