...
class TestProviderDiscovery:
    ...
    def test_save_memory_provider(self, tmp_path, monkeypatch):
        """Saving a memory provider keeps HermesUltimate vault-only."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        config_file = tmp_path / "config.yaml"
        config_file.write_text("memory:\n  provider: ''\n")
        ...
        assert content["memory"]["provider"] == ""
    ...
    def test_memory_provider_options_disabled(self):
        """HermesUltimate does not expose external memory providers."""
        from hermes_cli.plugins_cmd import _memory_provider_options_disabled
        assert _memory_provider_options_disabled() == []
