...
class TestWebServerEndpoints:
    ...
    def test_get_env_vars_includes_9router_default_provider(self):
        resp = self.client.get("/api/env")
        assert resp.status_code == 200
        data = resp.json()

        assert data["NINEROUTER_API_KEY"]["category"] == "provider"
        base = data["NINEROUTER_BASE_URL"]
        assert base["category"] == "provider"
        assert base["is_set"] is True
        assert base["is_default"] is True
        assert base["source"] == "default"

    def test_get_env_vars_reads_9router_from_stack_env(self):
        from hermes_cli.config import get_hermes_home

        stack_env = get_hermes_home() / "stack.env"
        stack_env.write_text(
            "NINEROUTER_API_KEY=stack-secret-key\n"
        ...
        assert data["NINEROUTER_API_KEY"]["is_set"] is True
        assert data["NINEROUTER_API_KEY"]["source"] == "stack_env"
        assert data["NINEROUTER_BASE_URL"]["is_set"] is True
        assert data["NINEROUTER_BASE_URL"]["source"] == "stack_env"
        assert data["NINEROUTER_BASE_URL"]["is_default"] is False
    ...
    def test_reveal_env_var_returns_9router_default(self):
        from hermes_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN

        resp = self.client.post(
            "/api/env/reveal",
            json={"key": "NINEROUTER_BASE_URL"},
            headers={_SESSION_HEADER_NAME: _SESSION_TOKEN},
        )
        assert resp.status_code == 200
        assert resp.json()["value"] == "http://localhost:20128/v1"
