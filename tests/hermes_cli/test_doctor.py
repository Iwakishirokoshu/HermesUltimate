...
class TestDoctorToolAvailabilityOverrides:
    def test_leaves_external_memory_provider_unavailable(self):
...
class TestDoctorMemoryProviderSection:
    ...
    def test_no_provider_shows_builtin_ok(self, monkeypatch, tmp_path):
        ...
        assert "Vault/built-in memory active" in out
        # Should NOT mention external provider errors
        assert "Honcho API key" not in out
        assert "Mem0" not in out

    def test_honcho_provider_configured_shows_disabled(self, monkeypatch, tmp_path):
        out = self._run_doctor_and_capture(monkeypatch, tmp_path, provider="honcho")
        assert "Memory Provider" in out
        assert "configured but disabled" in out
        assert "Honcho API key" not in out

    def test_mem0_provider_configured_shows_disabled(self, monkeypatch, tmp_path):
        out = self._run_doctor_and_capture(monkeypatch, tmp_path, provider="mem0")
        assert "Memory Provider" in out
        assert "configured but disabled" in out
        assert "Mem0 plugin" not in out
