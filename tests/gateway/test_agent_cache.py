...
class TestExtractCacheBustingConfig:
    ...
    def test_external_memory_keys_are_not_provider_specific(self):
        """Vault-only gateways must not expose external provider cache keys."""
        from gateway.run import GatewayRunner

        out = GatewayRunner._extract_cache_busting_config({"memory": {"provider": "honcho"}})

        assert out["memory.provider"] == "honcho"
        assert not any(key.startswith("honcho.") for key in out)

    def test_memory_provider_change_busts_signature(self):
        """The provider value remains a cache key even though plugins are disabled."""
        from gateway.run import GatewayRunner
