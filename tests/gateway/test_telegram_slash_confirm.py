...
class TestSendSlashConfirm:
    ...
    @pytest.mark.asyncio
    async def test_callback_uses_global_registry_when_adapter_state_missing(self, monkeypatch):
        adapter = _make_adapter()
        ...
        monkeypatch.setattr(
            slash_confirm,
            "find_session",
            lambda confirm_id: "session-1" if confirm_id == "cid4" else None,
        )
        ...
        assert calls == [("session-1", "cid4", "once")]
        assert answers == ["✅ Approved once"]
