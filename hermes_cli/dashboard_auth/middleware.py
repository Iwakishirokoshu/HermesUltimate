...
from hermes_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS, PUBLIC_API_PREFIXES
...
def _path_is_public(path: str) -> bool:
    ...
    * :data:`_GATE_PUBLIC_PREFIXES` — auth-bootstrap routes and static
    ...
    if path in PUBLIC_API_PATHS:
        return True
    if any(path == prefix or path.startswith(prefix) for prefix in PUBLIC_API_PREFIXES):
        return True
    return any(
