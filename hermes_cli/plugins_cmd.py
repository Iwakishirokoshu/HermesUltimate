...
def _memory_provider_options_disabled() -> list[tuple[str, str]]:
    """Return no external memory providers in HermesUltimate."""
    return []
...
def _get_current_memory_provider() -> str:
    """Return the active memory provider (always vault-only)."""
    return ""
...
def _save_memory_provider(name: str) -> None:
    """Persist vault-only memory.provider to config.yaml."""
    from hermes_cli.config import load_config, save_config
    config = load_config()
...
def _configure_memory_provider() -> bool:
    ...
    providers = _memory_provider_options_disabled()
