...
def main():
    ...
    # gateway souls — operator-side soul-router management (audit P1-4 / T-114).
    # Mirrors the in-chat /soul handler in gateway/run.py so souls can be
    # listed / inspected / switched / reset from the CLI as well.
    gateway_souls = gateway_subparsers.add_parser(
        "souls",
        help="Manage per-chat soul routing (list / get / set / reset)",
        description=(
            "Inspect and switch which soul a chat is routed to. Souls are "
            "loaded from souls/*.yaml; per-chat state lives in "
            "$HERMES_HOME/soul_state.db. Use 'set' to flip a chat onto a "
            "non-default soul (e.g. 'red' for the Decepticon backend) "
            "without having to send a /red message first."
        ),
    )
    gateway_souls.add_argument(
        "souls_action",
        nargs="?",
        default="list",
    ...
    gateway_souls.add_argument(
        ...
        dest="chat_id",
        default=None,
        help="Target chat id (required for get/set/reset)",
    )
    gateway_souls.add_argument(
        "--soul",
        dest="soul_name",
        default=None,
        help="Soul name to activate (required for set)",
    )
...
""",
    ...
    memory_parser = subparsers.add_parser(
        "memory",
        help="Manage built-in and vault memory",
        description=(
            "HermesUltimate keeps memory vault-only: built-in MEMORY.md / USER.md "
            "plus HermesVault. External memory provider plugins are disabled."
        ),
    )
    memory_sub = memory_parser.add_subparsers(dest="memory_command")
    _setup_parser = memory_sub.add_parser(
        "setup", help="Confirm vault-only memory configuration"
    )
    _setup_parser.add_argument(
        "provider",
        nargs="?",
        default=None,
        help="Ignored; external memory providers are disabled in HermesUltimate",
    )
    memory_sub.add_parser("status", help="Show vault-only memory status")
    memory_sub.add_parser("off", help="Keep vault-only memory mode active")
