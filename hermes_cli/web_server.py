...
from hermes_cli.web_api import DASHBOARD_ROUTERS

for _router in DASHBOARD_ROUTERS:
...
app.state.session_header_name = _SESSION_HEADER_NAME
...
from hermes_cli.dashboard_auth.public_paths import (
    ...
    PUBLIC_API_PREFIXES as _PUBLIC_API_PREFIXES,
)
...
async def auth_middleware(request: Request, call_next):
    ...
    public_prefix = any(path == prefix or path.startswith(prefix) for prefix in _PUBLIC_API_PREFIXES)
    if path.startswith("/api/") and path not in _PUBLIC_API_PATHS and not public_prefix:
        if not _has_valid_session_token(request):
            return JSONResponse(
                status_code=401,
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "memory.provider": {
        "type": "select",
        "description": "Memory provider (vault-only)",
        "options": ["builtin"],
    },
    "approvals.mode": {
        "type": "select",
...
def _apply_main_model_assignment(
    ...
    if provider_slug in {"ninerouter", "9router"}:
        provider = "custom"
        base_url = base_url.strip() or _ninerouter_base_url()
...
def get_model_options():
    ...
    try:
        ...
        return _merge_ninerouter_provider(payload)
    except Exception:
...
def get_auxiliary_models():
    ...
    try:
        ...
        for slot in _AUX_TASK_SLOTS:
            ...
            slot_provider = str(slot_cfg.get("provider", "auto") or "auto")
            slot_base_url = str(slot_cfg.get("base_url", "") or "")
            tasks.append({
                "task": slot,
                "provider": _provider_label_for_dashboard(slot_provider, slot_base_url),
        ...
        if isinstance(model_cfg, dict):
            ...
            main_base_url = str(model_cfg.get("base_url", "") or "")
            main = {
                "provider": _provider_label_for_dashboard(main_provider, main_base_url),
...
async def set_model_assignment(body: ModelAssignment):
    ...
    if provider.strip().lower() in {"ninerouter", "9router"}:
        provider = "custom"
        base_url = base_url or _ninerouter_base_url()
...
def _ensure_openai_v1_url(raw: Any) -> str:
...
def _ninerouter_base_url(env_on_disk: Optional[Dict[str, str]] = None) -> str:
    env_on_disk = env_on_disk or load_env()
    stack_env = _load_stack_env()
    return _ensure_openai_v1_url(
        env_on_disk.get("NINEROUTER_BASE_URL")
        or stack_env.get("NINEROUTER_BASE_URL")
        or env_on_disk.get("NINEROUTER_PUBLIC_URL")
        or stack_env.get("NINEROUTER_PUBLIC_URL")
        or env_on_disk.get("NINEROUTER_URL")
        or stack_env.get("NINEROUTER_URL")
        or os.getenv("NINEROUTER_BASE_URL")
        or os.getenv("NINEROUTER_PUBLIC_URL")
        or os.getenv("NINEROUTER_URL")
        or OPTIONAL_ENV_VARS.get("NINEROUTER_BASE_URL", {}).get("default")
        or "http://localhost:20128/v1"
    )


def _ninerouter_api_key(env_on_disk: Optional[Dict[str, str]] = None) -> str:
    env_on_disk = env_on_disk or load_env()
    stack_env = _load_stack_env()
    return (
        env_on_disk.get("NINEROUTER_API_KEY")
        or stack_env.get("NINEROUTER_API_KEY")
        or os.getenv("NINEROUTER_API_KEY")
        or ""
    ).strip()


def _provider_label_for_dashboard(provider: str, base_url: str = "") -> str:
    if provider.strip().lower() == "custom" and base_url:
        if _ensure_openai_v1_url(base_url).rstrip("/") == _ninerouter_base_url().rstrip("/"):
            return "9Router"
    return provider


def _fetch_ninerouter_models() -> dict[str, Any]:
    """Return live OpenAI-compatible models from the local 9Router service."""
    base_url = _ninerouter_base_url()
    url = f"{base_url.rstrip('/')}/models"
    headers = {"Accept": "application/json"}
    api_key = _ninerouter_api_key()
    if api_key:
    ...
    try:
        ...
        return {
            ...
            "base_url": base_url,
            "models": [],
            "error": f"9Router returned HTTP {exc.code}: {detail}",
        }
    except Exception as exc:
        return {
            "ok": False,
            "base_url": base_url,
            "models": [],
            "error": f"Could not reach 9Router at {url}: {exc}",
        }
    ...
    ids = _parse_model_ids(_ResponseShim())
    return {
        "ok": True,
        "base_url": base_url,
        "models": ids,
...
def _merge_ninerouter_provider(payload: dict[str, Any]) -> dict[str, Any]:
    live = _fetch_ninerouter_models()
    models = live.get("models") or []
    if not models:
        return payload

    providers = list(payload.get("providers") or [])
    base_url = str(live.get("base_url") or _ninerouter_base_url())
    current_provider = str(payload.get("provider") or "").lower()
    current_model = str(payload.get("model") or "")
    is_current = (
        current_provider in {"ninerouter", "9router"}
        or (current_provider == "custom" and current_model in set(models))
    )
    ...
    filtered = [
        ...
        if str(provider.get("slug", "")).lower() not in {"ninerouter", "9router"}
    ]
    payload["providers"] = [row] + filtered
    return payload


def _load_stack_env() -> Dict[str, str]:
    path = Path(os.getenv("HERMES_STACK_ENV") or (get_hermes_home() / "stack.env"))
    values: Dict[str, str] = {}
    if not path.exists():
        return values
    try:
...
def _env_default_value(
    var_name: str,
    info: Dict[str, Any],
    env_on_disk: Dict[str, str],
    stack_env: Optional[Dict[str, str]] = None,
) -> str:
    stack_env = stack_env or {}
    if var_name == "NINEROUTER_BASE_URL":
        return _ensure_openai_v1_url(
            env_on_disk.get("NINEROUTER_BASE_URL")
            or stack_env.get("NINEROUTER_BASE_URL")
            or env_on_disk.get("NINEROUTER_PUBLIC_URL")
    ...
    return str(info.get("default") or "").strip()
...
async def get_env_vars():
    ...
    for var_name, info in OPTIONAL_ENV_VARS.items():
        ...
        if not value and var_name.startswith("NINEROUTER_"):
            value = stack_env.get(var_name)
            source = "stack_env" if value else ""
        default_value = "" if value else _env_default_value(var_name, info, env_on_disk, stack_env)
        effective_value = value or default_value
        result[var_name] = {
            "is_set": bool(effective_value),
            "redacted_value": redact_key(effective_value) if effective_value else None,
            ...
            "is_default": bool(default_value),
            "source": source or ("default" if default_value else ""),
...
async def get_router_models():
    """Proxy 9Router's live model list through the dashboard backend."""
    live = _fetch_ninerouter_models()
    if not live.get("ok"):
        return {
            "ok": False,
            "base_url": live.get("base_url", _ninerouter_base_url()),
            "data": [],
            "models": [],
            "error": live.get("error", "9Router did not return models"),
        }
    data = live.get("data")
    if not isinstance(data, list):
        data = [{"id": model, "owned_by": "9router"} for model in live.get("models", [])]
    return {
        "ok": True,
        "base_url": live.get("base_url", _ninerouter_base_url()),
        "data": data,
...
async def remove_env_var(body: EnvVarDelete):
    try:
        ...
        if not removed:
            info = OPTIONAL_ENV_VARS.get(body.key)
            stack_env = _load_stack_env()
            if body.key.startswith("NINEROUTER_") and stack_env.get(body.key):
                return {"ok": True, "key": body.key, "removed": False}
            if info and _env_default_value(body.key, info, load_env(), stack_env):
                return {"ok": True, "key": body.key, "removed": False}
            raise HTTPException(status_code=404, detail=f"{body.key} not found in .env")
        return {"ok": True, "key": body.key, "removed": True}
    except HTTPException:
        raise
    except ValueError as exc:
...
async def reveal_env_var(body: EnvVarReveal, request: Request):
    ...
    if value is None:
        info = OPTIONAL_ENV_VARS.get(body.key)
        stack_env = _load_stack_env()
        if body.key.startswith("NINEROUTER_"):
            value = stack_env.get(body.key, "")
        if not value:
            value = _env_default_value(body.key, info or {}, env_on_disk, stack_env) if info else ""
        if not value:
            raise HTTPException(status_code=404, detail=f"{body.key} not found in .env")
...
async def get_memory_status():
    ...
    if isinstance(mem, dict):
        configured_provider = str(mem.get("provider") or "")
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        raise HTTPException(
            status_code=400,
            detail="External memory providers are disabled in HermesUltimate.",
        )
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from hermes_cli.plugins_cmd import (
        ...
        _memory_provider_options_disabled,
        _get_disabled_set,
        _get_enabled_set,
    ...
    try:
        for n, desc in _memory_provider_options_disabled():
