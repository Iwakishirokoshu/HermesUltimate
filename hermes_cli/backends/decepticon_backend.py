...
_ENGAGEMENT_SLUG_RE = re.compile(r"[^A-Za-z0-9_.-]+")


def normalize_engagement_slug(value: str | None, *, fallback: str = "hermes-red") -> str:
    """Return a Decepticon-safe engagement slug for /workspace/<slug> paths."""

    raw = str(value or "").strip()
    slug = _ENGAGEMENT_SLUG_RE.sub("-", raw).strip(".-_")
    if not slug:
        slug = fallback
    return slug[:96]


def workspace_path_for_engagement(slug: str) -> str:
    return f"/workspace/{normalize_engagement_slug(slug)}"
...
class DecepticonBackend(AgentBackend):
    ...
    def __init__(
        ...
        *,
        ...
        self.engagement_name = normalize_engagement_slug(engagement_name)
        self.workspace_path = workspace_path or workspace_path_for_engagement(self.engagement_name)
