...
@router.get("/read")
async def read_vault_file(path: str = Query(...)) -> dict[str, Any]:
...
async def search_vault(request: SearchRequest) -> dict[str, Any]:
    ...
    top_k = max(1, min(int(request.top_k or 20), 100))
    needle = query.lower()
    results: list[dict[str, Any]] = []

    for file_path in _markdown_files():
        content = _read_markdown(file_path)
        haystack = f"{_relative(file_path)}\n{content}".lower()
        index = haystack.find(needle)
        if index < 0:
            continue
        content_index = content.lower().find(needle)
        if content_index < 0:
            snippet = _summary(file_path)
        else:
            start = max(0, content_index - 90)
            end = min(len(content), content_index + len(query) + 140)
            snippet = content[start:end].replace("\n", " ").strip()
        results.append(
            {
                **_tree_entry(file_path),
                "snippet": snippet,
            }
        )
