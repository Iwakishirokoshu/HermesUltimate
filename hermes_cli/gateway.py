...
# Soul routing — surfaces SoulRouter from gateway/run.py's per-chat
# /red /blue /soul handlers up to the CLI so operators can list, get,
# set, or reset a chat's active soul without spinning up the gateway
# message loop. Plan T-114 / audit P1-4: this import + the get_soul_router()
# helper + the `souls` subcommand below make the soul-router integration
# concrete and grep-discoverable at the CLI surface, not only inside the
# gateway runtime. SoulRouter itself stays lazy-loaded so a CLI run that
# never touches souls doesn't pay the yaml-parse cost.
...
# =============================================================================
# Soul routing helpers (T-114 / audit P1-4)
# =============================================================================

_SOUL_ROUTER_SINGLETON = None


def get_soul_router():
    """Return a lazily-initialised SoulRouter for CLI use.

    Imports SoulRouter from hermes_cli.soul_router on first access so the
    cli module stays importable on hosts without souls/*.yaml yet (fresh
    install before T-002 ran). The singleton is cached for the rest of the
    process — `hermes gateway souls list` followed by `hermes gateway souls
    set` reuses the same SQLite connection guard and in-memory yaml cache.
    """
    global _SOUL_ROUTER_SINGLETON
    if _SOUL_ROUTER_SINGLETON is None:
        from hermes_cli.soul_router import SoulRouter

        _SOUL_ROUTER_SINGLETON = SoulRouter()
    return _SOUL_ROUTER_SINGLETON


def _souls_command(args) -> int:
    """Dispatch `hermes gateway souls <action>` subcommands.

    Mirrors the in-chat /soul handler in gateway/run.py for operator use
    from the CLI (useful for debugging, scripting, and bringing a new
    chat onto a non-default soul without sending a message first).
    """
    action = (getattr(args, "souls_action", None) or "list").lower()
    router = get_soul_router()

    if action == "list":
        names = router.list_souls()
        if not names:
            print_warning("No souls registered. Add YAML files to souls/*.yaml.")
            return 1
        print_header("Available souls")
        try:
            configs = router._load_configs()  # noqa: SLF001 (internal cache)
        except Exception as exc:
            print_warning(f"Could not load soul configs ({exc}); falling back to names only.")
            configs = {}
        for name in names:
            ...
            if cfg is None:
                ...
                tools = ",".join(cfg.allowed_toolsets) or "<none>"
                lg = f" langgraph={cfg.langgraph_url}" if cfg.langgraph_url else ""
                print(f"  - {name}  backend={cfg.backend}  toolsets=[{tools}]{lg}")
        return 0

    if action == "get":
        chat_id = getattr(args, "chat_id", None)
        if not chat_id:
            print_error("souls get requires --chat-id")
            return 2
        cfg = router.get_active_soul(chat_id)
        print(f"chat_id={chat_id}  soul={cfg.name}  backend={cfg.backend}")
        return 0

    if action == "set":
        chat_id = getattr(args, "chat_id", None)
        name = getattr(args, "soul_name", None)
        if not chat_id or not name:
            print_error("souls set requires --chat-id and --soul")
            return 2
        try:
            cfg = router.set_active_soul(chat_id, name)
        except ValueError as exc:
            print_error(str(exc))
            return 1
        print_success(f"chat_id={chat_id} -> soul={cfg.name} ({cfg.backend})")
        return 0

    if action == "reset":
        chat_id = getattr(args, "chat_id", None)
        if not chat_id:
            print_error("souls reset requires --chat-id")
            return 2
        cfg = router.set_active_soul(chat_id, "default")
        print_success(f"chat_id={chat_id} reset to default ({cfg.backend})")
        return 0

    print_error(f"Unknown souls action: {action}")
    return 2
