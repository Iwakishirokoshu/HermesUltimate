...
def run_doctor(args):
    ...
    if not _active_memory_provider:
        check_ok("Vault/built-in memory active", "(external memory providers disabled in HermesUltimate)")
    else:
        check_warn(
            f"External memory provider '{_active_memory_provider}' configured but disabled",
            "unset memory.provider; use MEMORY.md, USER.md, and HermesVault",
        )
    try:
