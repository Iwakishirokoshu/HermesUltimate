...
def list_authenticated_providers(
    ...
    # --- 5. Active bare custom endpoint ---
    # The VPS/installer can configure the local 9Router proxy as:
    #
    #   model:
    #     provider: custom
    #     base_url: http://localhost:20128/v1
    #
    # without also writing a providers:/custom_providers: entry.  That is a
    # valid runtime config, but the interactive picker used to show zero rows
    # because discovery only looked at named provider entries.  Surface the
    # active OpenAI-compatible endpoint directly so /model keeps the same
    # buttons UX as the CLI/dashboard after first install.
    current_base_url_norm = str(current_base_url or "").strip().rstrip("/").lower()
    has_current_custom = any(
        str(row.get("api_url") or "").strip().rstrip("/").lower() == current_base_url_norm
        for row in results
    )
    if (
        str(current_provider or "").strip().lower() == "custom"
        and current_base_url_norm
        and not has_current_custom
    ):
        models_list: list[str] = []
        try:
            from hermes_cli.models import fetch_api_models

            models_list = fetch_api_models("", str(current_base_url).strip().rstrip("/"))
        except Exception:
            models_list = []
        ...
        try:
            ...
            parsed = urlparse(str(current_base_url))
            hostport = f"{parsed.hostname or ''}:{parsed.port or ''}"
        except Exception:
            hostport = ""
        display_name = (
            "9Router Local"
            if hostport in {"localhost:20128", "127.0.0.1:20128"}
            else "Custom OpenAI Endpoint"
        )
        results.append({
            "slug": "custom",
            "name": display_name,
